...
@Component({
  selector: 'annualexams',  
  styleUrls: ['./app.component.css'],  
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncAnnualexams(true)">
...
export class AnnualexamsComponent implements OnInit{
aboutustextannualexams:string; 
...
linkShow= 'visible';
  ...
  myFuncAnnualexams(status){
    ...
    if(status) {
     ...
     this.aboutustextannualexams = this.productsService.annualExamsData();
      return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextannualexams, title: "Annual Exams", }, BSModalContext));
