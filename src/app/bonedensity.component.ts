...
@Component({
  selector: 'bonedensity',  
  styleUrls: ['./app.component.css'],  
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncBonedensity(true)">
...
export class BonedensityComponent implements OnInit{
aboutustextbonedensity:string; 
...
linkShow= 'visible';
  ...
  myFuncBonedensity(status){
    ...
    if(status) {
     ...
     this.aboutustextbonedensity = this.productsService.bonedensityData();
     return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextbonedensity, title: "Bone Density", }, BSModalContext));
