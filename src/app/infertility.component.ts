...
@Component({
  selector: 'infertility',  
  styleUrls: ['./app.component.css'],  
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncInfertility(true)">
...
export class InfertilityComponent implements OnInit{
aboutustextinfertility:string; 
...
linkShow= 'visible';
  ...
  myFuncInfertility(status){
    ...
    if(status) {
     ...
     this.aboutustextinfertility = this.productsService.infertilityData();
      return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextinfertility, title: "Infertility", }, BSModalContext));
