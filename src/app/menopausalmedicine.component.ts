...
@Component({
  selector: 'menopausalmedicine',  
  styleUrls: ['./app.component.css'],  
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncmenopausalmedicine(true)">
...
export class MenopausalmedicineComponent implements OnInit{
aboutustextmenopausalmedicineData:string; 
...
linkShow= 'visible';
  ...
  myFuncmenopausalmedicine(status){
    ...
    if(status) {
     ...
     this.aboutustextmenopausalmedicineData = this.productsService.menopausalmedicineData();
     return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextmenopausalmedicineData, title: "Menopausal Medicine", }, BSModalContext));
