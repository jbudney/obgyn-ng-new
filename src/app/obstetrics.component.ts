...
@Component({
  ...
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncObstetrics(true)">
...
export class ObstetricsComponent implements OnInit{
aboutustextobstetrics:string; 
...
linkShow= 'visible';
  ...
  myFuncObstetrics(status){
    ...
    if(status) {
     ...
     this.aboutustextobstetrics = this.productsService.obstetricsData();
      //this.linkHide = 'visible';
      //this.linkShow= 'hidden';
       
     return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextobstetrics, title: "Obstetrics", }, BSModalContext));
  
        } 
      else {
      this.aboutustextobstetrics = ""; 
      }
