...
@Component({
  selector: 'roboticsurgery',  
  styleUrls: ['./app.component.css'],  
  template:`<span class = "light" [style.visibility]='linkShow' (click)="myFuncRoboticsurgery(true)">
...
export class RoboticsurgeryComponent implements OnInit{
aboutustextoroboticsurgery:string; 
...
linkShow= 'visible';
  ...
  myFuncRoboticsurgery(status){
    ...
    if(status) {
     ...
     this.aboutustextoroboticsurgery = this.productsService.roboticsurgeryData();
     // + " " + "http://www.davincisurgery.com/surgeon-locator/doctor-profile.html?locale=en_US&ws=&sitekey=English-1&selector=surgeon&cmd=profile&id=119630";
      return this.modal.open(CustomModal,  overlayConfigFactory({ num1: 2, num2: 3,dataIn: this.aboutustextoroboticsurgery, title: "Robotic Surgery", }, BSModalContext));
